...
% should these be here, or before the section? (they also apply
% to the next section)
...
\bkey{take right hand side}{A$\vdash$B}
\bkey{null}{$\dashv$A}
\bkey{take left hand side}{A$\dashv$B}
...
\bkey{for each b$\in$B, apply: Ab}{A${}^{"}$B}
...
For {\tt A}$=8$, the sign before the square root is the opposite sign of $\tt B$.
